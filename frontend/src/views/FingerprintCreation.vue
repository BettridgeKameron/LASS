...
<template>
  <div id="app">
    <TitleComponent title="Lass-Fingerprint-Creation"></TitleComponent>
  </div>
  <div class="container mx-auto p-4 max-w-2xl">
    <h1 class="text-4xl text-center font-bold mb-2 py-10">Fingerprint Creation</h1>
    <div class="flex flex-col space-y-4">
      <input
        v-model="userInput"
        type="text"
        class="w-full border-2 border-gray-300 p-2 rounded-md"
        placeholder="Enter the fingerprint"
      />
      <input
        v-model="fingerprintName"
        type="text"
        class="w-full border-2 border-gray-300 p-2 rounded-md"
        placeholder="Enter the name for the Fingerprint"
        />
      <button
        @click="sendString"
